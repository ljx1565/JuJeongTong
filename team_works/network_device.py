...
class NetworkDevice:
    check_list = {
        "n_1": {"title": "비밀번호 설정"},
        "n_2": {"title": "비밀번호 복잡성 설정"},
        "n_3": {"title": "암호화된 비밀번호 사용"},
        "n_4": {"title": "계정 잠금 임계값 설정"},
        "n_6": {"title": "VTY 접근(ACL) 설정"},
        "n_7": {"title": "세션 종료 시간 설정"},
        "n_8": {"title": "VTY 접속 시 안전한 프로토콜 사용"},
        "n_9": {"title": "불필요한 보조 입출력 포트 사용 금지"},
        "n_11": {"title": "로그 서버 설정"},
        "n_13": {"title": "로그 버퍼 사이즈 설정"},
        "n_15": {"title": "NTP 및 시각 동기화 설정"},
    ...
    def n_2(self):
        pattern = r"^security\s+passwords\s+min-length\s+(\d+)$"
        ...
        for line in self.show_run.splitlines():
            ...
            if min_length >= 8:
                return (
                    "양호",
                    f"비밀번호 최소 길이가 {min_length}자로 설정되어 있습니다.",
                )

            return (
                "취약",
                f"비밀번호 최소 길이가 기준 미만입니다: {min_length}자",
            )

        return (
            "취약",
            "security passwords min-length 설정이 존재하지 않습니다.",
        )
    ...
    def n_3(self):
        ...
        for raw_line in self.show_run.splitlines():
            ...
            if re.match(r"^enable\s+secret(?:\s+\d+)?\s+\S+", line):
                enable_secret_ok = True

            if re.match(r"^enable\s+password(?:\s+\d+)?\s+\S+", line):
            ...
            if re.match(
                ...
                weak_settings.append("로컬 계정에 username password 방식 사용")

            if re.match(r"^password\s+\S+$", line):
                weak_settings.append("라인에 평문 password 사용")

            if re.match(r"^password\s+0\s+\S+$", line):
                weak_settings.append("라인에 Type 0 평문 password 사용")

        if service_encryption_ok and enable_secret_ok and not weak_settings:
            return (
                "양호",
        ...
        if not service_encryption_ok:
            reasons.append("service password-encryption 설정 없음")

        if not enable_secret_ok:
            reasons.append("enable secret 설정 없음")
    ...
    def n_4(self):
        ...
        for raw_line in self.show_run.splitlines():
            ...
            if block_time > 0 and 1 <= attempts <= 5 and within_time > 0:
                return (
                    "양호",
                    "계정 잠금 정책이 설정되어 있으며 "
            ...
            if attempts <= 0:
                ...
                reasons.append(f"실패횟수가 기준 초과: {attempts}회")
    ...
    def n_6(self):
        ...
        for raw_line in self.show_run.splitlines():
            ...
            if numbered_match:
                ...
                numbered_acls.setdefault(acl_number, []).append(acl_rule)
                current_vty = None
                current_named_acl = None
                continue
        ...
        for block in vty_blocks:
            ...
            for line in block:
                m = re.match(r"^access-class\s+(\S+)\s+in$", line)
            ...
            for rule in acl_rules:
                ...
                if normalized == "permit any":
                    unrestricted_permit = True

                if normalized == "permit 0.0.0.0 255.255.255.255":
                    unrestricted_permit = True
            ...
            if not permit_found:
                weak_reasons.append(f"ACL {acl_name}: permit 규칙 없음")

            if unrestricted_permit:
                weak_reasons.append(f"ACL {acl_name}: 전체 출발지 허용")
    ...
    def n_7(self):
        ...
        for block in line_blocks:
            ...
            if block_name.startswith("line aux ") and "no exec" in block:
                continue
            ...
            if not timeout_found:
                weak_reasons.append(f"{block_name}: exec-timeout 설정 없음")
    ...
    def n_8(self):
        ...
        if not ssh_version_2:
            weak_reasons.append("ip ssh version 2 설정 없음")
        ...
        for block in vty_blocks:
            ...
            for line in block:
                m = re.match(r"^transport\s+input\s+(.+)$", line)
    ...
    def n_9(self):
        ...
        for block in aux_blocks:
            ...
            if "no password" not in block:
                weak_reasons.append(f"{block_name}: no password 설정 없음")

            if "transport input none" not in block:
                weak_reasons.append(
                    f"{block_name}: transport input none 설정 없음"
                )

            if "no exec" not in block:
                weak_reasons.append(f"{block_name}: no exec 설정 없음")

            if "exec-timeout 0 1" not in block:
                weak_reasons.append(
            ...
            for line in block:
                ...
                if re.match(r"^password(?:\s+\d+)?\s+\S+", line):
                    weak_reasons.append(
    ...
    def n_11(self):
        ...
        for line in self.show_logging.splitlines():
            ...
            if re.match(r"^Syslog logging:\s+enabled", line):
                syslog_enable = True

            if re.match(r"^Logging to\s+\d{1,3}(\.\d{1,3}){3}", line):
                remote_logging = True
            ...
            if m:
                ...
                if trap_level in ok_trap_levels:
                    trap_level_ok = True

        if syslog_enable and remote_logging and trap_level_ok:
            return (
                "양호",
                "Syslog, Remote Logging 및 적절한 Trap level이 설정됨",
            )

        return (
            "취약",
            "Syslog, Remote Logging 또는 Trap level 설정 확인 필요",
        )
        ...
        for line in self.show_logging.splitlines():
            ...
            if m:
                ...
                if 16384 <= log_size <= 32768:
                    return (
                        "양호",
                        f"Log Buffer 사이즈 적정: {log_size} bytes",
                    )

                return (
                    "취약",
                    f"Log Buffer 사이즈 범위 초과: {log_size} bytes",
                )

        return (
            "취약",
            "Log Buffer 사이즈 미설정 또는 확인 불가",
        )
    ...
    def n_15(self):
        for line in self.show_run.splitlines():
            ...
            if re.match(r"^ntp\s+server\s+\S+", line):
                return (
                    "양호",
                    "NTP 서버가 설정되어 있습니다.",
                )
        ...
        return (
            "취약",
            "NTP 서버가 설정되어 있지 않습니다.",
        )

    def n_16(self):
        if (
            "clock timezone KST 9" in self.show_run
            and "service timestamps debug datetime msec localtime show-timezone"
            in self.show_run
            and "service timestamps log datetime msec localtime show-timezone"
            in self.show_run
        ):
            return (
                "양호",
                "시간대 및 타임스탬프 설정이 정상입니다.",
            )

        return (
            "취약",
            "시간대 또는 타임스탬프 설정이 미흡합니다.",
        )
    ...
    def n_17(self):
        if "snmp-server community" in self.show_run:
            return (
                "취약",
                "SNMP community 설정이 존재함",
            )

        return (
            "양호",
            "SNMP community 설정이 없음",
        )
    ...
    def n_18(self):
        ...
        for line in self.show_run.splitlines():
            ...
            if m:
                ...
                if re.search(r"[A-Za-z]", community_string):
                    combined_count += 1

                if re.search(r"[0-9]", community_string):
                    combined_count += 1

                if re.search(r"[^A-Za-z0-9]", community_string):
                    combined_count += 1

                if len(community_string) < 8 or combined_count < 2:
                    return (
                        "취약",
                        f"SNMP 커뮤니티 문자열 복잡도 미흡: "
                        f"{community_string}",
                    )

        return (
            "양호",
            "모든 SNMP 커뮤니티 문자열이 복잡도를 만족합니다.",
        )
    ...
    def n_21(self):
        for line in self.show_run.splitlines():
            if line.strip() == "service config":
                return (
                    "취약",
                    "자동 설정(service config) 기능이 활성화되어 있습니다.",
                )

        return (
            "양호",
            "자동 설정 기능이 비활성화되어 있습니다.",
        )
    ...
    def n_22(self):
        ...
        for line in self.show_run.splitlines():
            ...
            m = re.match(r"^ip access-group\s+(\S+)\s+in$", line)
        ...
        for line in self.show_run.splitlines():
            ...
            if not m or m.group(1) not in acl_names:
                continue
        ...
        if urpf_ok:
            return (
                "양호",
                "uRPF 기반 Spoofing 방지 설정이 적용되어 있습니다.",
            )

        if acl_applied and deny_found and permit_found:
            return (
                "양호",
                "Spoofing 방지 ACL이 인터페이스에 적용되어 있습니다.",
            )

        return (
            "취약",
            "Spoofing 방지 ACL 또는 uRPF 설정이 미흡합니다.",
        )

    def n_25(self):
        if (
            "service tcp-keepalives-in" in self.show_run
            and "service tcp-keepalives-out" in self.show_run
        ):
            return (
                "양호",
                "TCP Keepalive 설정이 활성화되어 있습니다.",
            )

        return (
            "취약",
            "TCP Keepalive 설정이 미흡합니다.",
        )

    def n_26(self):
        for line in self.show_run.splitlines():
            if line.strip() == "ip finger":
                return (
                    "취약",
    ...
    def n_27(self):
        if (
            "no ip http server" in self.show_run
            and "no ip http secure-server" in self.show_run
        ):
            return (
                "양호",
                "HTTP/HTTPS 서버가 비활성화되어 있습니다.",
            )

        return (
            "취약",
            "HTTP/HTTPS 서버 설정 확인이 필요합니다.",
        )
    ...
    def n_28(self):
        for line in self.show_run.splitlines():
            ...
            if (
                line == "service tcp-small-servers"
                or line == "service udp-small-servers"
            ):
                return (
                    "취약",
                    "불필요한 Small Server 서비스가 활성화되어 있습니다.",
                )

        return (
            "양호",
            "Small Server 서비스가 비활성화되어 있습니다.",
        )

    def n_29(self):
        if (
            "no ip bootp server" in self.show_run
            and "ip dhcp bootp ignore" in self.show_run
        ):
            return (
                "양호",
                "BOOTP 설정이 안전합니다.",
    ...
    def n_31(self):
        for line in self.show_run.splitlines():
            if line.strip() == "ip directed-broadcast":
                return (
                    "취약",
                    "Directed-broadcast가 활성화되어 있습니다.",
                )

        return (
            "양호",
            "Directed-broadcast가 비활성화되어 있습니다.",
        )
    ...
    def n_32(self):
        if "no ip source-route" in self.show_run:
            return (
                "양호",
                "Source-route가 비활성화되어 있습니다.",
            )

        return (
            "취약",
            "Source-route가 활성화되어 있습니다.",
        )

    def n_33(self):
        for intf in self._get_l3_int():
            if "no ip proxy-arp" not in intf or "ip proxy-arp" in intf:
                return (
                    "취약",
                    f"{intf[0]}에 Proxy ARP 차단 설정이 미흡합니다.",
                )

        return (
            "양호",
            "모든 L3 인터페이스에서 Proxy ARP가 비활성화되어 있습니다.",
        )

    def n_34(self):
        for intf in self._get_l3_int():
            if (
                "no ip unreachables" not in intf
                or "no ip redirects" not in intf
            ):
                return (
                    "취약",
                    f"{intf[0]}에 Unreachables/Redirects 설정이 미흡합니다.",
                )

        return (
            "양호",
            "모든 L3 인터페이스 설정이 양호합니다.",
        )

    def n_35(self):
        for line in self.show_run.splitlines():
            if line.strip() == "ip identd":
                return (
                    "취약",
                    "Identd 서비스가 활성화되어 있습니다.",
                )

        return (
            "양호",
            "Identd 서비스가 비활성화되어 있습니다.",
        )
    ...
    def n_38(self):
        for intf in self._get_l3_int():
            if "ip mask-reply" in intf:
                return (
                    "취약",
                    f"{intf[0]}에 Mask-reply가 활성화되어 있습니다.",
                )

        return (
            "양호",
            "모든 L3 인터페이스에서 Mask-reply가 비활성화되어 있습니다.",
        )
